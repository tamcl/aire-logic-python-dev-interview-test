...
def create_user(sqlite_db_path: str, user: User, table: str = USER_TABLENAME):
    ...
    if not check_user_exists(sqlite_db_path=sqlite_db_path, match=match_param, table=table):
...
def get_user(sqlite_db_path: str, match: dict, table: str = USER_TABLENAME) -> pd.DataFrame:
    query_param = dict(
        table=table,
        match=match
    )
...
def update_user(sqlite_db_path: str, user_login: dict, change: dict, table: str = USER_TABLENAME):
...
def validate_user(sqlite_db_path: str, match: dict, table: str = USER_TABLENAME) -> bool:
    """
    ...
    validate_user_query = jinja2.Environment().from_string(query).render(table=table, match_param=match_param)
